...
### 현재 확보 데이터 (17개 소스, ~100MB raw + ~20MB processed)
...
| **百度百科 W3** | `crawl-baidu-wave3.js` | **73건** (34도시+9관문+11제도+6군사+13행정) | 지리·제도·군사 |
...
| `crawl-baidu-wave3.js` | 百度百科 | W3: 도시34+관문9+제도11+군사6+행정13 |

모든 크롤러: `--resume` 재개, `--delay N` 요청간격(ms), 에러 시 graceful fallback.

### 프로토타입 검증 결과

**soul.md v2** — 426명 생성 완료 (`data/characters/`), 427명 soul-data 추출 완료 (`data/processed/soul-data/`)
...
| 평균 길이 | ~2,000자 | ~7,800자 |
| 회차 인용 | 0개 | 평균 12.5개 |
| 캐릭터 수 | — | **426명 soul.md** + 427명 soul-data (12섹션) |
...
100MB 원시 데이터(17소스, 23개 크롤러)를 게임 엔진이 소화할 수 있는 구조화된 데이터로 변환.
...
Phase B — LLM 생성 (Claude Opus, $0 — 세션 내 서브에이전트)
  P2 ──→ P8(Soul-data 추출, 427명) ──→ Soul.md 생성 (426명)
  P2 ──→ 이벤트 생성 (337개, 309 멀티초이스)

Phase C — 바이두 역주입 + 엔진 통합
  P9(바이두 관계) ──→ Soul.md 보강 (143명 관계+전투 이력)
  P10(바이두 전투) ──→ 이벤트 전투 매칭 (75건 battle_data)
  P5(지리확장) ──→ 엔진 시나리오 통합 (42도시 좌표/지형/자원)
```
...
| — | Soul.md v2 | 26× Claude Opus + 벌크 생성 | `data/characters/*.soul.md` (**426명**, 143명 바이두 보강) | ~3MB |
| — | 이벤트 생성 | 13× Claude Opus 서브에이전트 | `data/events/all-events.json` (337 이벤트, 309 멀티초이스, 75건 전투 연결) | ~1.5MB |
...
| P11 | Soul.md 역주입 | `enrich-soul-with-baidu.js` | 143명 soul.md 보강 (바이두 관계+전투 이력) | — |
| P12 | 이벤트 전투 매칭 | `enrich-events-with-battles.js` | 75건 battle_data 연결, 53건 전략 태깅 | — |
| P13 | 엔진 지리 통합 | `integrate-geography-engine.js` | 42도시 좌표/지형/자원 → 시나리오 | — |
...
- [x] 百度百科 Wave 3: 73/75건 (도시34+관문9+제도11+군사6+행정13) — **총 342 바이두 파일**
...
- [x] P7: Soul.md v2 — 426명 (73명 Claude Opus + 258명 벌크 + 143명 바이두 보강)
- [x] P8: **Soul-data 추출** — **427명** (12섹션: 능력치, 연의, 대사, 관계, 연표, 나무위키, EN Wiki, 정사, 백과사전, 정사영역, 멀티버전, 커뮤니티)
- [x] 이벤트 생성 — 337개 완전한 게임 이벤트 (event-schema 준수)
...
- [x] P11: Soul.md 바이두 역주입 — 143명 관계+전투 이력 보강
- [x] P12: 이벤트-전투 매칭 — 75건 battle_data, 53건 전략 태깅
- [x] P13: 엔진 지리 통합 — 42도시 좌표/지형/자원 동기화
...
│   ├── events/             # 게임 이벤트 (337개, 309 멀티초이스, 75건 전투 연결)
│   └── processed/          # 중간 산출물 (~20MB)
│       ├── character-profiles/  # 346개 통합 프로필 (14소스 퓨전)
│       ├── soul-data/           # 427개 soul-data (12섹션)
│       ├── relationship-graph.json  # 926 관계 엣지 (16타입)
│       ├── battles-structured.json  # 39 구조화 전투
│       ├── geography-expanded.json  # 42도시 + 57루트 + 8지형
