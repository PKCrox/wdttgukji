...
**soul.md 프로토타입** — 조조, 유비 2명 수작업 작성 완료 (`data/characters/`)

| 항목 | 조조 (cao-cao.soul.md) | 유비 (liu-bei.soul.md) |
|---|---|---|
| 능력치 총합 | 449 (S, 1위) | 399 (A, 20위권) |
| 기질 | ENTJ — 실용주의 올라운더 | ENFJ — 매력 편중 (99) 포섭형 |
| 의사결정 | 참모 청취 → 독단 결정 | 위임형, 감정 기반 폭발 가능 |
| 전략 패턴 | 우세 시 정공법, 열세 시 기습 | 열세 시 동맹·의탁, 장기 인내 |
| 인재 등용 | 적장도 능력이면 등용 시도 | 삼고초려식 집요한 구애 |
| 핵심 트리거 | 배신 → 감정적 보복 | 의형제 위기 → 판단력 상실 |

**검증 인사이트**: 두 캐릭터의 soul.md가 게임 내 행동으로 어떻게 분화되는지 확인됨.
- 조조 AI: 관우 포로 → 등용 시도 3회 → 실패 시 석방 (존경+미련)
- 유비 AI: 관우 사망 → 이릉 원정 발동 (의리 > 전략적 합리성)
- → **soul.md의 가치관 우선순위와 행동 트리거가 캐릭터 차별화의 핵심 축으로 기능**
...
- [ ] 장수 능력치 전체 파싱 (182→670+ 확장, Playwright 스크립트)
- [ ] 다국어 소스 (중국/일본) 크롤링
- [ ] 유저 커뮤니티 센티먼트 수집

### Phase 1: 기획 스키마 확정 + 콘텐츠 시드
- [ ] Tier 0 장수 20명 soul.md 수작업 완성 (프로토타입 기반 확장)
- [ ] AI soul.md 생성 파이프라인 구축 (Tier 1~3용)
