...
function buildPrompt({
  ...
  return [
    ...
    '- Good targets include: map interaction upgrades, selection/command flow, phaser overlay hierarchy, action-panel decision UX, keyboard-first controls, or tactical overlays.',
    '- Prefer real changes in src/scenes/*.js, src/screens/*.js, src/utils/*.js, index.html, or Phaser-facing config/data bridges.',
    '- Only touch public/old/** or generated public/** when you are explicitly working in the legacy surface lane.',
    ...
    'Verification:',
    '- Run node --check on each touched src/*.js file or helper script you changed.',
    '- Run npm run qa:slice or npm run qa:surface only in the outer lane, not inside this hook.',
    '- Do not run npm run qa:slice inside this Codex hook if the outer durable lane already owns that gate.',
